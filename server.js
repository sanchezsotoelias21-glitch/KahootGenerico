// =============================================================
//  TeamKahoot — server.js (LOCAL DEVELOPMENT)
//  Stack: Node.js + Express + Socket.io + PostgreSQL
//  
//  ⚠️  NOTA: Para Vercel, usa /api/index.js en su lugar
//  Este archivo se mantiene para desarrollo local con:
  ...
  try {
    ...
    res.json({
      ...
      user: { id: user.id, username: user.username, email: user.email,
              avatar: user.avatar, games_played: user.games_played, total_score: user.total_score }
    });
  } catch (e) {
...
/** GET /api/leaderboard  — Global top-10 users */
...
/** POST /api/moderator/verify  — Check moderator PIN */
...
io.on('connection', socket => {
  ...
  socket.on('create-room', async ({ topicId, moderatorName }) => {
    try {
      ...
      if (!topicRes.rows.length) { socket.emit('error', { msg: 'Tema no encontrado' }); return; }

      const questions = await fetchQuestions(topicId);
      if (!questions.length) { socket.emit('error', { msg: 'Sin preguntas para este tema' }); return; }
  ...
  socket.on('join-room', ({ roomCode, username, userId }) => {
    ...
    if (!room)                  { socket.emit('join-error', { msg: 'Sala no encontrada. Verifica el código.' }); return; }
    if (room.state !== 'waiting'){ socket.emit('join-error', { msg: 'El juego ya comenzó. Espera la próxima partida.' }); return; }
    if (!username || username.trim().length < 1) { socket.emit('join-error', { msg: 'Nombre de jugador requerido.' }); return; }

    // Unique name in room
    for (const [, p] of room.players)
      if (p.username.toLowerCase() === username.toLowerCase().trim())
        { socket.emit('join-error', { msg: 'Ese nombre ya está tomado en esta sala.' }); return; }

    // Alternate team assignment (A, B, A, B...)
    const team   = room.players.size % 2 === 0 ? 'A' : 'B';
    const avatarN = Math.floor(Math.random() * 8) + 1;
    const player  = {
      socketId: socket.id, userId: userId || null,
      username: username.trim(), team, score: 0,
      streak: 0, avatar: avatarN, joinedAt: Date.now()
    };
  ...
  socket.on('start-game', ({ roomCode }) => {
    ...
    if (room.players.size < 2) { socket.emit('error', { msg: 'Necesitas al menos 2 jugadores para comenzar.' }); return; }
  ...
  socket.on('submit-answer', ({ roomCode, questionId, answerIndex }) => {
    ...
    if (!player) return;
    if (room.qAnswers[socket.id]) return; // already answered
...
function sendQuestion(roomCode) {
  ...
  // Start timer with a 1 s head-start for rendering
  setTimeout(() => {
    ...
    room.timer = setInterval(() => {
      ...
      if (t <= 0) { clearInterval(room.timer); showResults(roomCode); }
    }, 1000);
  }, 1000);
}
...
function showResults(roomCode) {
  ...
  const playerResults = players.map(p => {
    const ans = room.qAnswers[p.socketId];
    return {
      username:    p.username,
      team:        p.team,
      avatar:      p.avatar,
      totalScore:  p.score,
      answerIndex: ans?.answerIndex ?? -1,
...
server.listen(PORT, () => {
  console.log(`🚀 TeamKahoot running on http://localhost:${PORT}`);
  console.log(`📋 Moderator PIN: ${process.env.MODERATOR_PIN || '1234'}`);
});
